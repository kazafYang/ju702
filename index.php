...
if ($conn->connect_error) {
    ...
    $sql = "select id,code,status,number,trade_type,trade_buy_price,trade_sell_price,stat_date,history_make_money from trade_history order by id desc limit 10;"; //where status=0 and stat_date='$stat_date'
    //echo $sql."\n";
    $result = $conn->query($sql);
	echo '<table border="1"><tr><th>id</th><th>code</th><th>status</th><th>trade_type</th><th>number</th><th>trade_buy_price</th><th>trade_sell_price</th><th>history_make_money</th><th>stat_date</th><</tr>';
	while($row=mysqli_fetch_array($result)){
	echo '<tr><td>'.$row[id].'</td><td>'.$row[code].'</td><td>'.$row[status].'</td><td>'.$row[trade_type].'</td><td>'.$row[number].'</td><td>'.$row[trade_buy_price].'</td><td>'.$row[trade_sell_price].'</td><td>'.$row[history_make_money].'</td><td>'.$row[stat_date].'</td></tr>';
	}
